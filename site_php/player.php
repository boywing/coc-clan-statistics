...
$content .= '<table class="table table-light" style="border-collapse: separate; border-spacing: 1px;border:1px solid black;" border=1>';
$content .= '<thead class="thead-dark"><th>' . $language["PL_TABLE_DATE"] . "</th><th>" . $language["PL_TABLE_WAR_TYPE"] . "</th><th>" . $language["PL_TABLE_ATT_CLAN"] . "</th><th>" . $language["PL_TABLE_ATT_TH"] . "</th><th>" . $language["PL_TABLE_DEF"] . "</th><th>" . $language["PL_TABLE_DEF_TH"] . "</th><th>" . $language["PL_TABLE_DEF_CLAN"] . "</th><th>" . $language["PL_TABLE_STARS"] . "</th><th>" . $language["PL_TABLE_DELTA"] . '</th></thead>';
$content .= "<tbody>";

$attack_sql = "SELECT startTime, war_type, defender_tag, 
        ...
        if (mysqli_num_rows($result) > 0)
            {
                while($attack = mysqli_fetch_assoc($result))
                    {
		      if ($attack['war_type'] == "CWL")
			{
			  $content .= "<tr style=\"background-color:rgb(255,255,200)\">";
			}
		      else
			{
			  $content .= "<tr style=\"background-color:rgb(220,220,255)\">";
			}
		      $content .= "<td>" . $attack['startTime'] . '</td>';
		      $content .= "<td>" . $attack['war_type'] . '</td>';			
		      ...
		      if ($attack['attack_stars'] == 3)
			{
			  $content .= "<td style=\"background-color:rgb(200,255,200)\">";
			}
		      else
			{
			  $content .= "<td>";
			}
		      $content .= $attack['attack_stars'] . "* " . $attack['destructionPercentage'] . "%</td><td>" . $attack['delta'] . "</td>";
		      $content .= "</tr>";		      
		    }
            }
        else
...
$content .= '<table class="table table-light" style="border-collapse: separate; border-spacing: 1px;border:1px solid black;" border=1>';
$content .= '<thead class="thead-dark"><th>' . $language['PL_TABLE_DATE'] . "</th><th>" . $language["PL_TABLE_WAR_TYPE"] . "</th><th>" . $language['PL_TABLE_DEF_CLAN'] . "</th><th>" . $language['PL_TABLE_ATT'] . "</th><th>" . $language['PL_TABLE_ATT_TH'] . "</th><th>" . $language['PL_TABLE_ATT_CLAN'] . "</th><th>" . $language['PL_TABLE_STARS'] . "</th><th>" . $language['PL_TABLE_DELTA'] . '</th></thead>';
$content .= "<tbody>";

$attack_sql = "SELECT startTime, war_type, attacker_tag, 
(SELECT name FROM clans WHERE tag=a.defender_clan) AS defender_clan_name, 
